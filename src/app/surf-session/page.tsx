...
const SurfExperiences: React.FC = () => {
  const [startDate, setStartDate] = useState(
    typeof window !== "undefined"
      ? localStorage.getItem("startDate") || format(subDays(new Date(), 7), "yyyy-MM-dd")
  ...
  const handleFilterReset = () => {
    ...
    localStorage.setItem("startDate", format(subDays(new Date(), 7), "yyyy-MM-dd"));
  ...
  const filteredSurfExperiences = data.filter((experience) => {
    ...
    //	return false;
    //}
    if (filterRating && filterRating > 0 && experience.mySurfRating?.rating !== filterRating) {
      return false;
    }
    return true;
  });
